...
class TambahPemilihPage extends StatefulWidget {
  @override
  _TambahPemilihPageState createState() => _TambahPemilihPageState();
}

class _TambahPemilihPageState extends State<TambahPemilihPage> {
  ...
  Future<void> _signUp() async {
    ...
    try {
      // Step 1: Check if the pemilih already exists
      var checkUrl = Uri.parse('http://localhost/flutter-login-signup/pemilih.php'); 
      var checkResponse = await http.post(checkUrl, body: {
        "nisn": nisn,
      });

      if (checkResponse.statusCode == 200) {
        var checkResponseBody = json.decode(checkResponse.body);

        if (checkResponseBody["status"] == "exists") {
          _showErrorDialog("NISN already exists. Please use a different NISN.");
          return;
        }
      } else {
      ...
      // Step 2: If no duplicate, proceed with registration
      ...
      if (response.statusCode == 200) {
        ...
        if (responseBody["status"] == "success") {
          ...
          _showSuccessDialog(
            ...
            () {
              Navigator.pushReplacementNamed(context, '/datapemilih');
            },
          );
        } else {
